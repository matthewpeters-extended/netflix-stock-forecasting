...
| git identity (`user.name`, `user.email`) | **Not configured**, so commits will fail until it is |
...
**This will ask for your Mac login password.** That is expected, because it needs administrator rights to
...
You do not currently have a `~/.zprofile`, so the first command creates it. Terminal.app starts
...
again, because the `PATH` change only applies to shells started after it.
...
- **Authenticate Git with your GitHub credentials?** → **Yes** (this is the important one, because it
...
git operations in 2021, so you need either this, a personal access token, or an SSH key. `gh auth
...
ones inside `.venv`, so you no longer need the full path.
...
`.venv/` is already in `.gitignore`, because virtual environments are never committed. Anyone cloning the
...
The TensorFlow line is the one that matters, because it is the only dependency with a real chance of
...
have an editor you like, since nothing in the project requires it.
